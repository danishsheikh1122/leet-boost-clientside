...
import { Moon, Sun, Activity, Code, ChevronRight } from 'lucide-react'
...
const SidePanelContent = () => {
  ...
  const [submissionStatus, setSubmissionStatus] = useState("Waiting for submission details...")
  ...
  const postToBigOCalcEndpoint = useCallback((code, language, submissionId, submissionCode) => {
  ...
  const BigOResult = ({ complexity }) => {
    ...
    const highlightComplexity = (text) => {
      return text.replace(/O$$[^)]+$$/g, match => `<span class="text-[#FFA116] font-bold">${match}</span>`);
    };
    ...
    return (
      <div className="bg-[#f3f3f3] dark:bg-[#363636] p-4 rounded-lg mb-6">
        ...
        <p className="text-lg mb-2" dangerouslySetInnerHTML={{ __html: highlightComplexity(`Time Complexity: ${complexity.time_complexity}`) }} />
        <p className="text-lg mb-2" dangerouslySetInnerHTML={{ __html: highlightComplexity(`Space Complexity: ${complexity.space_complexity}`) }} />
    ...
    useEffect(() => {
      ...
      const handleMessage = (message) => {
        if (message.action === "updatePopup" && message.result) {
          const complexityResult = typeof message.result === 'string' 
  ...
  const SubmissionDetails = ({ status, code, language, result }) => (
    <div className="space-y-4">
      ...
      <div className="bg-[#f3f3f3] dark:bg-[#363636] p-4 rounded-lg">
        ...
        <p><strong>Big-O Analysis:</strong> {result}</p>
      </div>
      <div className="bg-[#f3f3f3] dark:bg-[#363636] p-4 rounded-lg">
