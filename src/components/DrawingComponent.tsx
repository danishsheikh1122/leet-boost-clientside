...
const DrawingApp: React.FC = () => {
  ...
  const draw = (e: React.MouseEvent<SVGSVGElement>) => {
    if (activeTool === 0 && isDragging) {
      ...
      setOffset((prev) => ({ x: prev.x + deltaX, y: prev.y + deltaY }))
      setStartPoint({ x: e.clientX, y: e.clientY })
    } else if (activeTool === 2 && isDrawing) {
      // Draw tool
      const { offsetX, offsetY } = e.nativeEvent
      setElements((prevElements) => {
        const lastElement = prevElements[prevElements.length - 1]
  ...
  const handleTextInput = (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
    const newElements = [...elements]
    if (newElements[index] && newElements[index].type === "text") {
      newElements[index].text = e.target.value
      setElements(newElements)
      addToHistory(newElements)
    }
    ...
    switch (element.type) {
      case "path":
        return (
          <path
            key={index}
            d={`M ${element.path.map(([x, y]: number[]) => `${x} ${y}`).join(" L ")}`}
            fill="none"
            stroke={element.stroke}
            strokeWidth={element.strokeWidth}
            opacity={element.opacity}
