...
const Popup = () => {
  ...
  const [submissionId, setSubmissionId] = useState<string | number>("Waiting for submission...");

  // Listen for messages from the content script
  useEffect(() => {
    const handleMessage = (message: any) => {
      if (message.action === "updatePopup") {
        const { submissionId, submissionCode, language, result } = message;
        setSubmissionId(submissionId);
        setCode(submissionCode);
        setLanguage(language);
        setResult(result ? JSON.stringify(result, null, 2) : "Error calculating Big-O.");
      }
    };
  ...
  return (
    <div>
      <h1>LeetCode Submission</h1>
      <div id="status">{submissionId}</div>
